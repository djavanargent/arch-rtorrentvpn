...
if [[ $tun_exit_code != 0 ]]; then
    echo "[warn] 'tun' module not available, you will not be able to connect to ruTorrent or Privoxy outside of your LAN"
fi
...
if [[ $iptable_mangle_exit_code != 0 ]]; then
    echo "[warn] 'iptable_mangle' module not available, you will not be able to connect to ruTorrent or Privoxy outside of your LAN"
else
...
# accept input to flood port 3000
if [[ $ENABLE_FLOOD == "yes" || $ENABLE_FLOOD == "both" ]]; then
...
if [[ $iptable_mangle_exit_code == 0 ]]; then
    ...
    # accept output from flood port 3000
    if [[ $ENABLE_FLOOD == "yes" || $ENABLE_FLOOD == "both" ]]; then
...
# accept output from flood port 3000 - used for lan access
if [[ $ENABLE_FLOOD == "yes" || $ENABLE_FLOOD == "both" ]]; then
